...
class DataProcessor(HealthCheckMixin, CalendarMixin):
    ...
    def __init__(self):
        ...
        with self.__class__._lock:
            ...
            # DS-03: per-table 质量等级门控表（策略要求逐表达标时使用）
            self._quality_tier_by_table: dict[str, int] = {}
    ...
    async def sync_stock_basic(self):
        ...
        try:
            ...
            # DAT-08③：移除 Phase 3F-2 轨道 A 写时替换。stock_basic.industry 始终保存
            # Tushare API 原始值；申万二级行业改由 screener_dao 查询时经 LATERAL join
            # 计算为 industry_sw_l2 列，两套分类不再混合。反向 v1.7.0 S4 / v1.9.0 M-4 决策。

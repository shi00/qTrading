...
class ScreeningHistory(Base):
    ...
    __table_args__ = (
        # RV-03: 研究记录本质是「某次运行的当时判断」，天然 append-only——唯一键改
        # (trade_date, strategy_name, ts_code, run_id)（由 LIFE-03 的 3 键覆盖语义演进）。
        # 同一天多次参数运行各自独立成行（params_snapshot/ai_reason/thinking 不互相覆盖），
        # 统计口径取最近一次运行（DISTINCT ON ... ORDER BY id DESC，见 screener_dao）。
        UniqueConstraint(
            "trade_date",
            "strategy_name",
            "ts_code",
...
class LimitList(Base):
    ...
    industry = Column(String)
    amount = Column(Numeric(20, 4))
    limit_amount = Column(Numeric(20, 4))
    float_mv = Column(Numeric(20, 4))
    total_mv = Column(Numeric(20, 4))
    turnover_ratio = Column(Numeric(12, 4))

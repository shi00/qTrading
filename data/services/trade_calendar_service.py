...
class TradeCalendarService:
    ...
    def _to_date(self, d) -> Optional[datetime.date]:
        """
        统一日期类型转换。
    ...
    def _to_str(self, d) -> Optional[str]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                if not trade_df.empty:
                    dates = pd.to_datetime(trade_df["cal_date"]).dt.date.tolist()
                    return sorted(dates)
    ...
    async def get_prev_trade_date(self, date) -> Optional[datetime.date]:
        """
        获取指定日期的上一个交易日。
    ...
    async def get_next_trade_date(self, date) -> Optional[datetime.date]:
        """
        获取指定日期的下一个交易日。
    ...
    async def get_latest_trade_date(self) -> Optional[datetime.date]:
        """
        获取最近的交易日。
    ...
    async def get_trade_dates_batch(
        ...
        trade_dates_set = set(all_trade_dates)

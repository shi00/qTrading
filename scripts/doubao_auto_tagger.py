...
class DoubaoTagger:
    ...
    async def get_pending_stocks(self, limit: int, exclude_codes: List[str] = None) -> List[Tuple[str, str]]:
        """从数据库异步获取尚未打标的股票"""
        exclude_codes = exclude_codes or []
        async with self.cm.engine.connect() as conn:
            query = """
                SELECT ts_code, name 
                FROM stock_basic sb 
                WHERE list_status = 'L' 
                  AND NOT EXISTS (
                      ...
                      WHERE sc.ts_code = sb.ts_code AND sc.concept_id LIKE 'AI_DOUBAO_%'
                  )
            """
            params = []
            if exclude_codes:
                placeholders = ",".join(["?"] * len(exclude_codes))
                query += f" AND ts_code NOT IN ({placeholders})"
    ...
    async def process_batch(self, page: Page, stocks: List[Tuple[str, str]]) -> bool:
        ...
        try:
            ...
            for _ in range(30):
                ...
                bubbles = await page.locator('.content-wrapper, div[data-testid="chat-message-bubble"]').all()
                potential_json = ""
        ...
        if response_text:
            try:
                ...
                print(f"✅ 成功解析 JSON，共提取了 {len(data)} 个股票对象。", flush=True)
                
                if self.dry_run:
                    print(f"🏜️ [Dry-Run] 模式已开启，跳过入库过程。提取结果如下：\n{json.dumps(data, ensure_ascii=False, indent=2)}", flush=True)
                    return True
            ...
            try:
                ...
                with open(os.path.join(APP_ROOT, "doubao_dom_debug.html"), "w", encoding="utf-8") as f:
                    f.write(html_content)
                await page.screenshot(path=os.path.join(APP_ROOT, "debug_doubao_fail.png"))
            except Exception:
                pass
            return False
    ...
    async def run(self, limit: int = 0):
        if not os.path.exists(AUTH_FILE):
            print(f"❌ 找不到持久化文件 {AUTH_FILE}！请先运行 doubao_login.py 登录。", flush=True)
            return

        await self.initialize()
        print(f"🚀 启动 Playwright 全自动打标流水线 (Refactored)...", flush=True)
        ...
        async with async_playwright() as p:
            ...
            while True:
                ...
                if limit > 0:
                    ...
                    if batch_limit <= 0:
                        print(f"✅ 已达到指定的测试上限 {limit} 只股票，任务结束。", flush=True)
                        break
                else:
                    batch_limit = random.randint(30, 50)
                
                stocks = await self.get_pending_stocks(batch_limit, exclude_codes)

                if not stocks and not exclude_codes:
                    print("✅ 数据库中所有正常股票已完成打标！全自动任务圆满结束。", flush=True)
                    break
                    ...
                    if not exclude_codes:
                        ...
                        break
                    stocks = await self.get_pending_stocks(20, [])  # 降低批次大小攻坚
                    # 过滤只查错题本中的股票
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dry-run', action='store_true', help='Do not save to databases')

...
| 3.2.5 | [lane:gate][tdd:required] `BacktestConfigPanel` 声明式重写：`@ft.component` + 复用 `BacktestViewModel` 或新建子 VM；移除命令式模式 | grep 命令式模式 = 0；测试通过 | Phase 3.0 | cc:完了 [PENDING] (纯 UI 状态组件用 use_state 不建 VM YAGNI；提取 _get_config_from_state 纯函数 11 单测；删除 _strategy_key/set_strategy_key 死代码；DatePicker use_ref+use_effect 生命周期；BacktestView 移除 set_strategy_key 调用 + refresh_locale 不再级联 config_panel；test_views.py 补 BacktestView mock；2414 UI 单测 green, ruff/pyright clean) |

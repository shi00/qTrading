...
| 3.2.3 | [lane:gate][tdd:required] `LLMConfigPanel` 声明式重写：`@ft.component` + 新建 `LLMConfigPanelViewModel`（provider/key/test）；移除命令式模式 | grep 命令式模式 = 0；测试通过 | Phase 3.0 | cc:完了 [e32a672] (469 tests green, ruff/pyright clean, DoD grep=0, VM 84 单测 + 10 契约守护 + 5 旧 API 契约测试替换 + ai_brain_tab save_config 收敛 + onboarding_wizard 适配) |
| 3.2.4 | [lane:gate][tdd:required] `LocalModelConfigPanel` 声明式重写：`@ft.component` + 新建 `LocalModelConfigPanelViewModel`（model/path/verify/save，FilePicker 生命周期由 use_effect 管理）；移除命令式模式 | grep 命令式模式 = 0；测试通过 | Phase 3.0 | cc:完了 [66e55e6] (7692 tests passed, ruff/pyright clean, DoD grep=0, VM 单测 + 8 契约守护 + 2 _render_message 纯函数 + onboarding_wizard/ai_brain_tab 消费方适配；注：采用建 VM 模式而非原注 YAGNI 不建 VM，因需承载 model 验证/路径管理/保存等业务逻辑) |
| 3.2.5 | [lane:gate][tdd:required] `BacktestConfigPanel` 声明式重写：`@ft.component` + 复用 `BacktestViewModel` 或新建子 VM；移除命令式模式 | grep 命令式模式 = 0；测试通过 | Phase 3.0 | cc:完了 [b9d211a] (纯 UI 状态组件用 use_state 不建 VM YAGNI；提取 _get_config_from_state 纯函数 11 单测；删除 _strategy_key/set_strategy_key 死代码；DatePicker use_ref+use_effect 生命周期；BacktestView 移除 set_strategy_key 调用 + refresh_locale 不再级联 config_panel；test_views.py 补 BacktestView mock；2414 UI 单测 green, ruff/pyright clean) |
| 3.2.6 | [lane:gate][tdd:required] `BacktestResultPanel` 声明式重写：`@ft.component` + `use_state`（纯展示，props 推送数据）；移除命令式模式 | grep 命令式模式 = 0；测试通过 | Phase 3.0 | cc:完了 [e486c3d] (@ft.component + use_state selected_tab/trades_page + props 推送 result/chart_min_height；移除命令式 set_chart_min_height/set_result；ft.Tabs/TabBar/TabBarView V1 三件套 API；BacktestView 消费方适配 props 推送模式 _refresh_result_panel 重新实例化；test_backtest_result_panel 重写为纯函数单测 + 契约守护；test_backtest_view_splitter 完整重写 yield fixture 持久化 patch + mock _result_container；2411 UI 单测 green, ruff/pyright clean, DoD grep=0) |
| 3.2.7 | [lane:gate][tdd:required] `StockDetailDialog` + `HealthReportDialog` 声明式重写：`@ft.component` + 条件渲染（Phase 3.0.2 模式）；移除 `page.show_dialog` | `grep "page.show_dialog" ui/components/stock_detail_dialog.py ui/components/health_report_dialog.py` = 0；测试通过 | Phase 3.0 | cc:完了 [24d5468] (StockDetailDialog + HealthReportDialog 重写为 @ft.component + use_state(open)/ft.use_dialog 条件渲染；子组件纯函数化 _health_dialog_size/_log_report_summary/_build_health_content；消费方 screener_view + data_source_tab 改 props 推送；test_health_report_dialog 39 tests + test_stock_detail_dialog 96 tests + screener_view 消费方测试适配；ruff/format clean, pyright 0 errors, 7666 passed / 20 既有失败与改动无关) |

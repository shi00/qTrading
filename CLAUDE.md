...
- **Flet 版本号引用（GDR-06）**：治理文档（CLAUDE.md / CONTRIBUTING.md / docs/flet/**）SHALL NOT 硬编码 Flet 补丁版本号（`x.y.z` 形式）；该门禁按行扫描、对正文与代码块同样拦截，需引用版本号时以「主版本.次版本」等相对描述代替（由 `check_docs_consistency.py` 的 Flet 版本漂移检查守护）。

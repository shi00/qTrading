"""应用会话入口（review01-A7 自 main.py 迁移）。

main.py 收敛为"日志初始化 + ft.run"入口；本模块承载启动编排全部逻辑：
i18n/窗口/主题/Toast 初始化 → embedded PG 场景检测 → 数据库就绪（含重试环）→
CacheManager/ShutdownCoordinator/WindowDialogManager → StartupController →
RootView 渲染 → controller.start()。

遵循宪法 §4.1：app 层编排所有层，仅被 main.py 调用。
"""
...
async def _prepare_db_with_retry(
    ...
    Raises:
        CancelledError: 用户在错误页关窗口或退避等待期间，run(page) 协程被取消，
...
async def run(page: ft.Page):
    """应用会话入口（review01-A7 自 main.py::main(page) 迁移，逻辑不变）。

    由 main.py 的 ``ft.run(main=app.application.run)`` 调用；``setup_logging()`` 与
    全局异常钩子由 main.py 入口负责。
    """
    _trace_log("[Main] run(page) entered")
    ...
    # ContextVar 是 per-asyncio-task 的，run(page) task 销毁时上下文即 GC；

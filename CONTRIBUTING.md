...
>
> **对应版本**：0.11.0（产品版本，与 pyproject.toml 一致，与 [CLAUDE.md](./CLAUDE.md) 保持一致）<!-- x-release-please-version -->
...
> 项目使用 pre-commit hooks（Ruff lint/format、裸 `type: ignore` 检测、禁止 `IsolatedAsyncioTestCase`、禁临时文件入库、pyright 增量类型检查（staged `.py`）、弱断言增量扫描（staged `test_*.py`，与 CI baseline 一致）、E2E anchor 引用校验、WCAG 主题对比度检查（ui/theme.py 变更时触发）、requirements 同步、版本一致性校验、文档一致性校验、红线自动化校验、import-linter 架构守护），完整 hook 清单与英文 id 见 [ci-cd.md「Pre-commit Hooks」](./docs/guides/ci-cd.md#pre-commit-hooks)（名称级正本，`repo: local` 下 16 个），名称与数量均以 [`.pre-commit-config.yaml`](./.pre-commit-config.yaml) 为唯一正本。
>
...
| 改动范围 | 至少运行 |
|---------|----------|
| 仅 Markdown/治理文档 | `python scripts/check_docs_consistency.py` + 相关文档契约测试（`python -m pytest tests/unit/test_docs_consistency.py`）；若修改 redlines/schema/policy，再运行对应治理测试；无需无差别运行 pyright、业务单测和 E2E |
...
data/             ← 数据层 (DAO、同步策略、外部数据源、领域服务、缓存管理)
  ...
  ├── persistence/       持久化 (DAOs、ORM 模型、数据库管理/迁移/配置、质量门控、配置/状态/元数据/复盘服务、内置 PostgreSQL 引擎 embedded_postgres/)
...
services/         ← 应用服务层 (AI 服务、任务管理器、本地模型管理、新闻订阅服务、回测服务、内置 PG 维护服务)

...
class TestPolarsFilterOffloadsToCPUPool(unittest.IsolatedAsyncioTestCase):
    """Verify filter() delegates collect+to_pandas to ThreadPoolManager.run_async(TaskType.CPU, ...)."""

    async def test_filter_calls_run_async_with_cpu_task_type(self):
        ...
        with patch.object(
            ...
            with patch.object(ThreadPoolManager, "run_async", new_callable=AsyncMock) as mock_run_async:
                ...
                self.assertEqual(call_args[0][0], TaskType.CPU)

    async def test_filter_run_async_receives_callable(self):
        ...
        with patch.object(
            ...
            with patch.object(ThreadPoolManager, "run_async", new_callable=AsyncMock) as mock_run_async:
                ...
                self.assertTrue(callable(callable_arg), "Second argument to run_async must be callable")

    async def test_filter_run_async_callable_produces_expected_dataframe(self):
...
class TestPolarsFilterWithMockedThreadPool(unittest.IsolatedAsyncioTestCase):
    """Verify filter() works correctly when ThreadPoolManager.run_async returns a valid DataFrame."""

    async def test_filter_returns_dataframe_from_run_async(self):
    ...
    async def test_filter_returns_empty_when_run_async_returns_empty(self):
...
class TestPolarsFilterEmptyInputSkipsRunAsync(unittest.IsolatedAsyncioTestCase):
    """Verify filter() returns early on empty input without calling run_async."""

    async def test_empty_screening_data_does_not_call_run_async(self):
    ...
    async def test_none_screening_data_does_not_call_run_async(self):
    ...
    async def test_unready_dependencies_does_not_call_run_async(self):

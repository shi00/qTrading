...
def mock_thread_pool():
    """Patch data_source_tab 模块级 ThreadPoolManager，run_async 直接同步执行。"""
    mock_tpm = MagicMock()
...
class TestOnTushareSave:
    def test_saves_valid_token_delegates_to_run_task(self, tab):
        """_on_tushare_save 应通过 page.run_task 异步委托执行保存。"""
        tab._on_tushare_save({"token": "abc123"})
        tab.page.run_task.assert_called_once()
        args = tab.page.run_task.call_args.args
        assert args[0].__name__ == "_do_tushare_save_async"
        assert args[1] == "abc123"

    def test_saves_token_stripped_delegates_to_run_task(self, tab):
        """_on_tushare_save 应将 token strip 后传递给异步方法。"""
        tab._on_tushare_save({"token": "  abc  "})
        tab.page.run_task.assert_called_once()
        assert tab.page.run_task.call_args.args[1] == "abc"

    def test_skips_empty_token(self, tab):
        tab._on_tushare_save({"token": ""})
        tab.page.run_task.assert_not_called()
    ...
    def test_skips_when_no_page(self, tab):
        tab.page = None
        tab._on_tushare_save({"token": "abc123"})
        # Should not raise; no run_task call possible

    @pytest.mark.asyncio
    async def test_do_tushare_save_async_saves_and_shows_snack(self, tab, mock_thread_pool):
        """_do_tushare_save_async 应通过 ThreadPoolManager offload 保存并显示成功提示。"""
    ...
    @pytest.mark.asyncio
    async def test_do_tushare_save_async_handles_exception(self, tab, mock_thread_pool):
        """_do_tushare_save_async 在保存失败时应记录错误并显示错误提示。"""
        tab.vm.save_tushare_token.side_effect = RuntimeError("save failed")
        await tab._do_tushare_save_async("abc123")
        tab.show_snack.assert_called_once()
...
class TestOnHistoryYearsChange:
    def test_valid_change_delegates_to_run_task(self, tab):
        """on_history_years_change 应通过 page.run_task 异步委托执行。"""
        e = MagicMock()  # spec omitted: Flet control event, dynamic attribute access
        e.control.value = "5"
        tab.on_history_years_change(e)
        tab.page.run_task.assert_called_once()
        args = tab.page.run_task.call_args.args
        assert args[0].__name__ == "_do_history_years_change_async"
        assert args[1] is e
    ...
    @pytest.mark.asyncio
    async def test_do_history_years_change_async_valid(self, tab, mock_thread_pool):
        """_do_history_years_change_async 应通过 ThreadPoolManager offload 保存并更新 UI。"""
        e = MagicMock()  # spec omitted: Flet control event, dynamic attribute access
        e.control.value = "5"
    ...
    @pytest.mark.asyncio
    async def test_do_history_years_change_async_invalid_logs_error(self, tab, mock_thread_pool):
        """_do_history_years_change_async 在值无效时应记录错误且不调用 set_history_years。"""
        e = MagicMock()  # spec omitted: Flet control event, dynamic attribute access
        e.control.value = "invalid"
        await tab._do_history_years_change_async(e)
        tab.vm.set_history_years.assert_not_called()
        mock_thread_pool.run_async.assert_not_called()

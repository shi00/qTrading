...
def test_flet_mock_page(mock_page):
    ...
    assert hasattr(mock_page, "shared_preferences")

    assert len(mock_page.controls) >= 1

    mock_page.shared_preferences.set("test_key", "test_value")
    assert mock_page.shared_preferences.get("test_key") == "test_value"

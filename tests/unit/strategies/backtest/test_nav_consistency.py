...
核心原则：
1. total_value = cash + raw_market_value（盯市用原始价格）
...
class TestNavRawConsistency:
    """total_value 必须使用 raw 口径"""
    ...
    def test_total_value_uses_raw_market_value(
        self,
        config: BacktestConfig,
        cost_model: TransactionCostModel,
    ) -> None:
        """
        total_value = cash + raw_market_value，而非 qfq_market_value。

        验证持仓记录中 total_value 与 raw 口径一致。
        """
        ...
        expected_total = cash + raw_mv_sum

        assert total_value == pytest.approx(expected_total, rel=1e-6)

    def test_total_value_not_using_qfq(
        self,
        config: BacktestConfig,
        cost_model: TransactionCostModel,
    ) -> None:
        """
        total_value 不应等于 cash + qfq_market_value。

        当 raw != qfq 时，验证 total_value 使用 raw 口径。
        """
        simulator = PortfolioSimulator(config, cost_model)
        ...
        qfq_based_total = cash + qfq_mv_sum

        assert total_value != pytest.approx(qfq_based_total, rel=0.1)
...
class TestExRightNavNoJump:
    ...
    def test_nav_continuous_on_ex_right(
        ...
        assert abs(daily_return_day2) < 0.5
...
class TestDailyReturnConsistency:
    ...
    def test_daily_return_from_raw_nav(self) -> None:
        """
        ...
        assert float(daily_returns[0]) == 0.0

...
async def test_config_and_calendar():
    ...
    rough_start = (datetime.datetime.now() - datetime.timedelta(days=int(250 * years * 1.5))).strftime('%Y%m%d')
    ...
    if all_dates:
        ...
        print(f"Start date calculated for {years} years: {start_date}. Available dates: {len(all_dates)}")
    else:

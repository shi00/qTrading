...
class TestAlembicMigrationAlignment:
    ...
    def test_top_list_alembic_alignment(self):
        ...
        assert not missing_in_alembic, (
            f"Alembic missing columns for top_list: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for top_list: {extra_in_alembic}"
        )
    ...
    def test_block_trade_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find block_trade in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for block_trade: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for block_trade: {extra_in_alembic}"
        )
    ...
    def test_limit_list_alembic_alignment(self):
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for limit_list: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for limit_list: {extra_in_alembic}"
        )
    ...
    def test_suspend_d_alembic_alignment(self):
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for suspend_d: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for suspend_d: {extra_in_alembic}"
        )
    ...
    def test_daily_quotes_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find daily_quotes in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for daily_quotes: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for daily_quotes: {extra_in_alembic}"
        )
    ...
    def test_moneyflow_daily_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find moneyflow_daily in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for moneyflow_daily: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for moneyflow_daily: {extra_in_alembic}"
        )
    ...
    def test_stock_basic_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find stock_basic in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for stock_basic: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for stock_basic: {extra_in_alembic}"
        )
    ...
    def test_dividend_alembic_alignment(self):
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for dividend: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for dividend: {extra_in_alembic}"
        )
    ...
    def test_daily_indicators_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find daily_indicators in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for daily_indicators: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for daily_indicators: {extra_in_alembic}"
        )
    ...
    def test_financial_reports_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find financial_reports in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for financial_reports: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for financial_reports: {extra_in_alembic}"
        )
    ...
    def test_margin_daily_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find margin_daily in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for margin_daily: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for margin_daily: {extra_in_alembic}"
        )
    ...
    def test_northbound_holding_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find northbound_holding in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for northbound_holding: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for northbound_holding: {extra_in_alembic}"
        )
    ...
    def test_index_daily_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find index_daily in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for index_daily: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for index_daily: {extra_in_alembic}"
        )
    ...
    def test_index_dailybasic_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find index_dailybasic in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for index_dailybasic: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for index_dailybasic: {extra_in_alembic}"
        )
    ...
    def test_index_weight_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find index_weight in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for index_weight: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for index_weight: {extra_in_alembic}"
        )
    ...
    def test_moneyflow_hsgt_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find moneyflow_hsgt in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for moneyflow_hsgt: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for moneyflow_hsgt: {extra_in_alembic}"
        )
    ...
    def test_fina_audit_alembic_alignment(self):
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for fina_audit: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for fina_audit: {extra_in_alembic}"
        )
    ...
    def test_fina_forecast_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find fina_forecast in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for fina_forecast: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for fina_forecast: {extra_in_alembic}"
        )
    ...
    def test_fina_mainbz_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find fina_mainbz in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for fina_mainbz: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for fina_mainbz: {extra_in_alembic}"
        )
    ...
    def test_pledge_stat_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find pledge_stat in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for pledge_stat: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for pledge_stat: {extra_in_alembic}"
        )
    ...
    def test_repurchase_alembic_alignment(self):
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for repurchase: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for repurchase: {extra_in_alembic}"
        )
    ...
    def test_shibor_daily_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find shibor_daily in migration script"
        )
        orm_to_db_mapping = {
            "w1": "1w",
            "w2": "2w",
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for shibor_daily: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for shibor_daily: {extra_in_alembic}"
        )
    ...
    def test_stk_holdernumber_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find stk_holdernumber in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for stk_holdernumber: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for stk_holdernumber: {extra_in_alembic}"
        )
    ...
    def test_top10_holders_alembic_alignment(self):
        ...
        assert alembic_cols is not None, (
            "Could not find top10_holders in migration script"
        )
        missing_in_alembic = model_cols - alembic_cols
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for top10_holders: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for top10_holders: {extra_in_alembic}"
        )
    ...
    def test_trade_cal_alembic_alignment(self):
        ...
        extra_in_alembic = alembic_cols - model_cols - {"updated_at", "created_at"}
        assert not missing_in_alembic, (
            f"Alembic missing columns for trade_cal: {missing_in_alembic}"
        )
        assert not extra_in_alembic, (
            f"Alembic has extra columns for trade_cal: {extra_in_alembic}"
        )

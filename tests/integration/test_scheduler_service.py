...
class TestSchedulerServiceSingleton:
    ...
    P2-2 fix: 直接构造真实 SchedulerService，调用生产 _reset_singleton()，
    而非测试内重新定义的 MockSchedulerService。
    """

    def _make_real_instance(self):
        """创建真实 SchedulerService 实例（绕过 __init__ 避免创建 AsyncIOScheduler）。"""
        from utils.scheduler_service import SchedulerService

        SchedulerService._reset_singleton()
        instance = SchedulerService.__new__(SchedulerService)
        SchedulerService._instance = instance

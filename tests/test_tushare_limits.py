    ...
    def test_fina_indicator_batch_permission(self):
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                print("This might mean the period is invalid or no data exists, but Permission is likely OK.")
            else:
                print("API returned None (Unknown error).")

        except Exception as e:
            print(f"API Failed with error: {e}")
            print("CONCLUSION: Batch sync MIGHT BE RESTRICTED. Fallback to Incremental/Looping mode needed.")

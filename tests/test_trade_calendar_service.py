...
class TestTradeCalendarService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_is_trading_day_fallback_to_api(self):
        ...
        await self.cache.stock_dao._write_db(
            "DELETE FROM trade_cal WHERE cal_date = $1",
            (test_date,)
        )
    ...
    async def test_get_start_date_by_trade_days(self):
        ...
        self.assertLess(result, datetime.date(2024, 3, 21))
    ...
    async def test_get_latest_trade_date(self):
        ...
        with patch(
            "data.services.trade_calendar_service.get_now"
        ) as mock_now:
    ...
    async def test_to_date_conversion(self):
        ...
        self.assertEqual(
            self.service._to_date("2024-03-21"),
            datetime.date(2024, 3, 21)
        )
        self.assertEqual(
            self.service._to_date("20240321"),
            datetime.date(2024, 3, 21)
        )
        self.assertEqual(
...
class TestTradeCalendarServiceEdgeCases(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_start_date_by_trade_days_exceeds_available(self):
        ...
        self.assertIsNotNone(result)
        self.assertLess(result, datetime.date(2024, 3, 22))
    ...
    async def test_get_prev_trade_date_first_available(self):
        ...
        self.assertIsNotNone(result)
        self.assertLess(result, datetime.date(2024, 3, 18))
    ...
    async def test_get_next_trade_date_last_available(self):
        ...
        self.assertIsNotNone(result)
        self.assertGreater(result, datetime.date(2024, 3, 22))
...
class TestTradeCalendarServiceOffline(unittest.IsolatedAsyncioTestCase):
...
class TestTradeCalendarServiceErrorHandling(unittest.IsolatedAsyncioTestCase):
...
class TestTradeCalendarServiceConcurrency(unittest.IsolatedAsyncioTestCase):
...
class TestTradeCalendarServiceBatch(unittest.IsolatedAsyncioTestCase):
...
class TestTradeCalendarServiceHolidays(unittest.IsolatedAsyncioTestCase):
    ...
    async def _seed_holiday_calendar(self):
        ...
        holidays = {
            datetime.date(2024, 2, 9), datetime.date(2024, 2, 10),
            datetime.date(2024, 2, 11), datetime.date(2024, 2, 12),
            datetime.date(2024, 2, 13), datetime.date(2024, 2, 14),
            datetime.date(2024, 2, 15), datetime.date(2024, 2, 16),
            datetime.date(2024, 2, 17), datetime.date(2024, 2, 18),
...
class TestTradeCalendarServiceYearBoundary(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_year_end_trading_day(self):
        ...
        self.assertIsNotNone(result)
        self.assertLessEqual(result.year, 2023)
    ...
    async def test_year_start_trading_day(self):
        ...
        self.assertIsNotNone(result)
        self.assertGreaterEqual(result, datetime.date(2024, 1, 1))


class TestTradeCalendarServicePersistence(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_api_data_is_persisted_to_db(self):
        ...
        api_df = pd.DataFrame({
            "cal_date": ["20240318", "20240319", "20240320", "20240321", "20240322"],
            "is_open": [1, 1, 1, 1, 1],
            "exchange": ["SSE"] * 5,
        })
        self.mock_api.get_trade_cal.return_value = api_df
...
class TestTradeCalendarServiceCacheTTL(unittest.IsolatedAsyncioTestCase):
...
class TestTradeCalendarServiceDatabaseTypes(unittest.IsolatedAsyncioTestCase):
...
class TestTradeCalendarServiceIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncTearDown(self):
        ...
        DataProcessor._is_initialized = False
    ...
    async def test_dataprocessor_mounts_trade_calendar(self):
        """Test that DataProcessor correctly mounts TradeCalendarService."""
        from data.data_processor import DataProcessor
        
        dp = DataProcessor()
        
        self.assertIsNotNone(dp.trade_calendar)
        self.assertIsInstance(
            dp.trade_calendar,
            TradeCalendarService
        )
    
    async def test_dataprocessor_trade_calendar_is_functional(self):
        """Test that TradeCalendarService from DataProcessor is functional."""
        from data.data_processor import DataProcessor
        
        dp = DataProcessor()
        
        result = await dp.trade_calendar.is_trading_day(datetime.date(2024, 3, 21))
        
        self.assertIsInstance(result, bool)


class TestTradeCalendarServiceGetTradeCalDf(unittest.IsolatedAsyncioTestCase):
    """Test get_trade_cal_df method for raw DataFrame access."""

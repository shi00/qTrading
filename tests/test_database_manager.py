...
class TestDatabaseManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cursor.execute("INSERT INTO stock_basic VALUES ('000001.SZ', 'PingAn'), ('600519.SH', 'Moutai')")
        
        cursor.execute("CREATE TABLE daily_quotes (ts_code TEXT, trade_date TEXT, close REAL)")
        for i in range(100):
            date = f"202301{i:02d}" if i < 31 else "20230201"
            cursor.execute(f"INSERT INTO daily_quotes VALUES ('000001.SZ', '{date}', {10.0 + i})")
    ...
    def test_execute_sql_valid(self):
        result = self.db_manager.execute_sql("SELECT * FROM stock_basic WHERE name = 'PingAn'")

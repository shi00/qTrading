...
`Control.before_event(e)` 在任何事件处理器之前被调用，返回 `False` 可取消该事件。适合做统一鉴权、路由守卫、埋点。**源码依据**：`flet/controls/base_control.py` 的 `Control._trigger_event()` 方法在调用 `on_<event>` 处理器前先调用 `self.before_event(e)`，返回值为 `False` 时跳过事件处理器（0.85.3 实测核实）。

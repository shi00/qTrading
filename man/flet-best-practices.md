...
本文件是 **项目 Flet 差异与升级清单**，仅记录 AStockScreener 相对 Flet 官方默认的**分叉点**、**项目验证过的高风险 API** 与**升级检查清单**。通用 Flet 教程（路由、Services、存储、构建打包、移动/Web 适配、响应式布局、控件清单等）请直接查阅 [Flet 官方文档](https://docs.flet.dev/)，本文件不再复制，避免与上游漂移。

**优先级**（后者被前者覆盖）：

1. [CLAUDE.md](../CLAUDE.md) — 项目宪法（红线 R1~R18、架构边界、交互准则）
2. [CONTRIBUTING.md](../CONTRIBUTING.md) — 项目实现规范（含「V1 声明式 UI 开发规范」）
3. **本文件** — Flet 项目差异与升级清单
...
项目锁定三个包：`flet` / `flet-desktop` / `flet-charts`，具体版本以 `pyproject.toml` 为准。本文件不写补丁版本号，避免与 `pyproject.toml` 漂移；升级时同步更新本文件「最后验证日期」。
...
| 声明式 Dialog | `ft.use_dialog()` Hook | **`ft.use_dialog()` Hook**（声明式组件内唯一契约） | §4.1 |
| Dropdown 事件 | `on_change` | **`on_select`** | §4.2 |
| `use_effect` cleanup | setup 返回 cleanup 函数 | **显式 `cleanup=` 参数传入** | §4.3 |
| 异步阻塞段 | `asyncio.to_thread` / `page.run_thread` | **`ThreadPoolManager.run_async(TaskType.IO/CPU)`** | R16 红线（§5） |
| 响应式断点 | xs/sm/md/lg/xl/xxl 576~1400 | **compact/standard/ultra_wide 1200/1600/2400** | [CONTRIBUTING.md](../CONTRIBUTING.md) 响应式断点 |
| 桌面打包 | `flet pack` | **PyInstaller**（`AStockScreener.spec`） | [CONTRIBUTING.md](../CONTRIBUTING.md) PyInstaller 小节 |
...
Flet 版本升级时，按以下清单逐项验证。

### 6.1 API 兼容性检查

- [ ] `ft.use_dialog()` 签名与行为（声明式 Dialog 唯一契约）
- [ ] `ft.Dropdown.on_select` 事件名（项目统一用 `on_select`，非 `on_change`）
- [ ] `use_effect` 的 `cleanup=` 关键字参数（非 setup 返回值）
...
- [ ] 检查 [CONTRIBUTING.md](../CONTRIBUTING.md)「V1 声明式 UI 开发规范」中 API 约束
- [ ] 更新本文件「最后验证日期」

# Flet 0.85.3 应用开发最佳实践手册

> 面向工程团队的架构与工程实践指南
> 适用版本：**Flet 0.85.3**（属于 Flet 1.0 Beta 系列，即 `v1` 现代架构）
> 最后更新：2026-07
...
>
> **项目专属约束覆盖本手册的 7 处分叉**（遇到冲突以项目规范为准）：
>
> | 维度 | 本手册（通用） | 项目规范（优先） |
> |------|--------------|----------------|
...
>
> 本手册中 Web/移动专属内容（WASM/CDN、APK/IPA 构建、SafeArea、Cupertino `adaptive`、移动端 NavigationBar 等）**项目桌面端不适用**，仅作背景知识。本手册 API 声明已对 `flet==0.85.3` 实测核实。
...
**极其重要的前提**：`0.85.3` 属于 Flet 从零重写的 **v1 架构**（1.0 Beta，起始于 `0.70`/`0.80`），与旧版 `0.28.x`（v0）**不兼容**。网上大量教程、旧博客、旧代码仍基于 v0，直接套用会报错。本手册所有实践均以 v1 为准。

> **范式约定（务必先读）**：Flet 0.85.3 主推 **声明式（Declarative / Reactive）** 开发方式（`@ft.component` + Hooks）。为保证一致性、可测试性与可维护性，**本手册只讲声明式，不涉及命令式写法**。请将"改状态、让组件重渲染"作为唯一心智模型，不要手动持有控件引用再修改其属性。

判断你看到的资料是否适用 v1 的快速信号：

| 特征 | v0（旧，≤0.28.x） | v1（本手册，≥0.70，含 0.85.3） |
| --- | --- | --- |
...
`Control.before_event(e)` 在任何事件处理器之前被调用，返回 `False` 可取消该事件。适合做统一鉴权、路由守卫、埋点。

---

## 4. 声明式编程模型（唯一推荐范式）

Flet 0.85.3 的现代主线是**声明式 / 响应式**。核心思想：用 `@ft.component` 定义函数组件，组件根据当前**状态**返回一棵 UI 树；状态变化时组件自动重渲染，框架做高效差量更新。你只需描述"状态 → UI 的映射"，永远不手动增删或修改控件对象。
...
| `use_dialog()` | 声明式管理对话框开关生命周期 | 自定义 hook |
...
**机制说明（v1 专属）**：v1 事件处理器默认**批量更新**——状态 setter 不会立即触发渲染，而是等到事件处理器返回时一次性刷新。`yield` 的作用是**主动让出事件循环**，触发一次中间帧渲染，是 v1 官方认可的长任务 UI 刷新方案（不是 Python 生成器语义的副作用，而是框架对 async 事件处理器的特殊支持）。适用场景：长任务需要在等待期间先把"处理中"等中间状态显示给用户，否则用户会看到界面"卡住"直到任务完成。
...
- 显示：`page.show_dialog(dialog_control)`（取代 `page.open()`）；
- 关闭最顶层：`page.pop_dialog()`。

适用于对话框、Banner、SnackBar 等弹层（抽屉 Drawer 走 `show_drawer`/`close_drawer`，见 §10.2）。**声明式首选 `ft.use_dialog()` Hook** 管理对话框打开/关闭生命周期；在组件事件里触发时，用 `ft.context.page` 获取页面再调用 `show_dialog` / `pop_dialog`：
        ...
        page.show_dialog(
            ft.AlertDialog(
                ...
                actions=[
                    ...
                    ft.FilledButton("删除", on_click=lambda e: page.pop_dialog()),
                ],
            )
        )

    return ft.FilledButton("删除", on_click=confirm)
```
...
> 不要再用 V0 的 `page.open(drawer)` / `page.close(drawer)`（已移除，抛 `AttributeError`）；统一用上面的 `show_drawer` / `close_drawer`。对话框/Banner/SnackBar 等其它弹层仍走 `page.show_dialog()` / `page.pop_dialog()`。
...
- **桌面窗口控制**：通过 `page.window` 管理窗口尺寸与行为，常用：
  ...
  - 需要启动即隐藏窗口时用 `ft.AppView.FLET_APP_HIDDEN` + `page.window.visible`（0.85.3 修复了 Windows 启动闪窗问题）。
...
0.85.3 会把 `flet-build-template.zip` 按版本缓存到 `$FLET_CACHE_DIR/build-template/v<version>/`（默认 `~/.flet/cache/`），并把 `FLET_CACHE_DIR` 透传给 Gradle，显著减少 Android 每次构建的"Creating app shell"耗时。CI 中应**缓存 `~/.flet/cache/` 目录**以复用。

### 13.5 桌面打包 `flet pack`

`flet pack` 用于桌面单文件打包；注意 Windows/Linux 打包在旧版本曾有客户端归档缺失问题，0.85 已修复，建议基于 0.85.3 打包。
...
## 17. 迁移提示：从 v0（0.28.x）到 v1（0.85.3）
...
- [ ] `pyproject.toml` 中精确 pin `flet==0.85.3` 及所有扩展版本；
...
- [ ] 弹层统一 `show_dialog`/`pop_dialog`；间距用 `Padding/Margin/BorderRadius` 类方法；

...
def _collect_cloud_ack_providers(primary_provider: str) -> list[str]:
    """SEC-01 复核修复: 主 provider + failover 不同云端 provider 的确认对象集合。

    run_ai_analysis 的外发确认必须覆盖实际可能触达的每个云端 provider：主 provider
    调用失败时 failover 会切换到 fallback（如 deepseek → qwen），若 fallback 未确认，
    数据会外发给未授权对象（授权语义漂移，SEC-01 缺口 1 的 failover 变体）。提取逻辑
    与 AIService failover 凭证预载（services/ai_service/__init__.py）一致：仅提取带
    ``/`` 前缀的 fallback provider（裸 model 名视为主 provider 内部模型，已由主确认
    覆盖）。failover 配置读取失败时降级为仅主 provider（failover 不可用时行为一致）。
    """
    ...
    try:
        ...
        logger.debug("[AIStrategyMixin] failover config unavailable (%s); ack scope = primary only", exc)
    return providers
...
class AIStrategyMixin:
    ...
    async def run_ai_analysis(
        ...
        ack_providers = _collect_cloud_ack_providers(current_provider)

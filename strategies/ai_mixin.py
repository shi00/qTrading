...
class AIStrategyMixin:
    ...
    async def run_ai_analysis(
        ...
        if ai_client.client is None:
            logger.info("[AIStrategyMixin] AI service not configured — returning math-only results")
            if on_progress:
                on_progress(0, 0, I18n.get("ai_not_configured", "AI 服务未配置，仅展示数学筛选结果"))
        ...
        logger.info(f"[AIStrategyMixin] Pre-fetched capital data: moneyflow={len(moneyflow_df)}, top_list={len(top_list_df)}, northbound={len(northbound_df)}")
        ...
        for row in candidates_df.itertuples(index=False):
            if dp and dp.is_cancelled():
                logger.info("[AIStrategyMixin] Cancellation detected — stopping remaining tasks")
                break
            ...
            try:
                ...
                if on_progress:
                    on_progress(completed_count, total_tasks,
                                I18n.get("ai_progress_analyzed", name=stock_name,
                ...
                logger.error(f"[AIStrategyMixin] Task error for {stock_name}: {e}", exc_info=True)
                completed_count += 1
            finally:
    ...
    async def _mixin_analyze_single(self, row: dict, dp, ai_client: AIService,
        ...
        try:
            ...
            capital_flow_text = self._build_capital_flow_text(ts_code, prefetched_capital or {})
            ...
            ai_result = await ai_client.analyze_stock(
                stock_info, tech_context, news, global_context,
            ...
            logger.error(f"[AIStrategyMixin] Analysis failed for {row.get('ts_code', '?')}: {e}")
            return None
    ...
    def _compute_technical_structure(history_df) -> dict:
        ...
        try:
            ...
            if ma5 is not None and ma10 is not None and ma20 is not None:
                if ma5 > ma10 > ma20:
                    result['ma_alignment'] = f'Bullish (MA5={ma5:.2f} > MA10={ma10:.2f} > MA20={ma20:.2f})'
                elif ma5 < ma10 < ma20:
                    result['ma_alignment'] = f'Bearish (MA5={ma5:.2f} < MA10={ma10:.2f} < MA20={ma20:.2f})'
                else:
                    result['ma_alignment'] = f'Mixed (MA5={ma5:.2f}, MA10={ma10:.2f}, MA20={ma20:.2f})'
            ...
            if 'vol' in df.columns and len(df) >= 10:
                ...
                if vol_10d > 0:
                    ...
                    if vol_ratio < 0.7:
                        result['volume_trend'] = f'Shrinking (5d/10d ratio: {vol_ratio:.2f})'
                    elif vol_ratio > 1.3:
                        result['volume_trend'] = f'Expanding (5d/10d ratio: {vol_ratio:.2f})'
                    else:
                        result['volume_trend'] = f'Stable (5d/10d ratio: {vol_ratio:.2f})'
                else:
            ...
            if len(df) >= 5:
                ...
                result['price_trend_5d'] = f'{pct_5d:+.1f}% over 5 days (closes: {closes_5d})'
            else:
                result['price_trend_5d'] = 'Insufficient data'

        except Exception as e:
            logger.warning(f"[AIStrategyMixin] Technical structure computation failed: {e}")
    ...
    def _build_history_text(history_df: pd.DataFrame) -> str:
        ...
        try:
            ...
            if has_pct_chg:
                ...
                consec_str = f"Consecutive {'Up' if sign_last > 0 else 'Down'} for {consec_days} days" if consec_days > 1 else "Consolidation"
            ...
            if has_vol:
                ...
                if pd.isna(vol_5d_avg): vol_5d_avg = 0.0
                if pd.isna(vol_older_avg): vol_older_avg = 0.0
                vol_ratio_5d = vol_5d_avg / vol_older_avg if vol_older_avg > 0 else 1.0
                vol_desc = "Significant Expansion" if vol_ratio_5d > 1.5 else "Significant Contraction" if vol_ratio_5d < 0.7 else "Flat"
    ...
    def _build_financials_text(row: dict) -> str:
        ...
        parts.append(f"总市值: {fmt(row.get('total_mv'), '万元', '.0f')}")

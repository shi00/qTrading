...
class BaseStrategy(ABC):
    ...
    def get_dynamic_description(self, current_params: dict) -> Message:
        ...
        By default, it just returns the static i18n key + params as a Message.
        Override in subclasses if the description needs to reflect slider values.

        Returns a ``Message`` (i18n key + params) instead of a translated string,
        so that VM state stays locale-independent (CLAUDE.md §3.2: VM 只产出 i18n key,
        View 按当前 locale 渲染). View renders via ``I18n.get(msg.key, **msg.params)``.

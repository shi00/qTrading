...
``_build_stale_section``）与 ``PreFetchedContext`` 数据容器。
按数据域分文件：
...
- ``technical.py``：``_compute_technical_structure``

涨跌停幅度/状态判定（``get_limit_pct`` / ``classify_limit_status``）正本已迁至
``utils.limit_status``（横切叶子），本子包不再承载该业务语义。
...
from strategies.ai_context.technical import _compute_technical_structure

...
[![UI](https://img.shields.io/badge/UI-Flet%20%3E%3D1.0-00d2b4)](https://github.com/shi00/qTrading)
...
* **仓位管理**: 支持等权、市值加权、按信号排名加权等多种仓位分配策略。
...
* Python 3.13+
* PostgreSQL 16.14.0+（外置模式需要；内置模式由应用自带 PostgreSQL 16.14.0，无需用户安装）
...
* **内置模式（默认，推荐新用户）**：应用自带 PostgreSQL 16.14.0 sidecar，首次启动自动准备本地数据库，无需任何配置。
* **外置模式（高级）**：连接用户自建的 PostgreSQL 16.14.0+ 实例，需在 `.env` 文件或环境变量中配置 `DATABASE_URL`。
...
**积分档位**：Tushare 按 token 积分档位（120/2000/5000/10000/15000）开放不同 API 与 QPS。应用启动时会执行 capability probe 探测当前 token 可访问的 API 集，未授权 API 在 UI 上灰显并跳过同步。积分档位映射配置真相源见 `utils/constants.py` 的 `TUSHARE_POINT_TIERS`（`data/constants.py` 中的同名常量仅为兼容再导出）。

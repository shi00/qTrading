...
[![UI](https://img.shields.io/badge/UI-Flet%200.86.2-00d2b4)](https://github.com/shi00/qTrading)
...
* Python 3.13+
* PostgreSQL 17.2.0+（外置模式需要；内置模式由应用自带 PostgreSQL 17.2.0，无需用户安装）
...
* **内置模式（默认，推荐新用户）**：应用自带 PostgreSQL 17.2.0 sidecar，首次启动自动准备本地数据库，无需任何配置。
* **外置模式（高级）**：连接用户自建的 PostgreSQL 17.2.0+ 实例，需在 `.env` 文件或环境变量中配置 `DATABASE_URL`。

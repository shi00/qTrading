"""ExternalPgForm — 声明式外部 PostgreSQL 配置表单 (P3-9).

P3-9 将原 DatabaseConfigPanel 中的表单部分 (host/port/user/password/database
/create_if_not_exists + test/save 按钮 + status display) 拆分到此模块,
原 DatabaseConfigPanel 改为路由容器按 is_embedded_mode() 切换。

CLAUDE.md §3.2 MVVM + §3.3 use_viewmodel hook:
- 外部 VM 模式 (vm=DatabaseConfigPanelViewModel): VM 由消费方实例化
- View 通过 use_viewmodel(vm=vm) 订阅 vm.state 变化触发重渲染
...
def _on_test_click_factory(vm: DatabaseConfigPanelViewModel) -> Callable[[ft.ControlEvent], None]:
    ...
    def _on_test_click(e: ft.ControlEvent) -> None:
        try:
            ...
            logger.debug("[ExternalPgForm] page not available for test_connection")
...
def _on_save_click_factory(vm: DatabaseConfigPanelViewModel) -> Callable[[ft.ControlEvent], None]:
    ...
    def _on_save_click(e: ft.ControlEvent) -> None:
        try:
            ...
            logger.debug("[ExternalPgForm] page not available for save_config")
...
def ExternalPgForm(
    ...
    *,
    ...
    P3-9 从原 DatabaseConfigPanel 拆分, 仅处理 external 模式的表单渲染。
    DatabaseConfigPanel 作为路由容器按 is_embedded_mode() 切换到此组件。

    CLAUDE.md §3.2 MVVM + §3.3 use_viewmodel hook:
    - VM 由消费方实例化 (DatabaseTab/OnboardingWizard/DatabaseConfigPanel router)

...
def BackupRestorePanel() -> ft.Container:
    ...
    # Step 3 (offline_guidance): 显示离线恢复指引 (D36)
    ...
    if state.confirm_state == "offline_guidance":
        ...
        # guidance 文案含 {backup_path} 占位符，通过 _render_message(state.progress_message) 渲染 (P1-4)
        restore_section_controls.append(
            ft.Text(
        ...
        restore_section_controls.append(
            ft.Text(
                _render_message(state.progress_message),
                size=AppStyles.FONT_SIZE_BODY_SM,
                color=AppColors.TEXT_PRIMARY,
            )
        )

...
class ToastCard(ft.Container):
    ...
    def _toggle_expand(self, e):
        ...
        if self.is_expanded:
            ...
            self.expand_btn.icon = ft.Icons.KEYBOARD_ARROW_UP  # type: ignore[untyped]
            self.expand_btn.tooltip = I18n.get("common_collapse")  # type: ignore[untyped]
            # Timer logic handled in start_timer loop
        else:
            self.text_control.max_lines = self.COLLAPSED_MAX_LINES
            self.expand_btn.icon = ft.Icons.KEYBOARD_ARROW_DOWN  # type: ignore[untyped]
            self.expand_btn.tooltip = I18n.get("common_expand")  # type: ignore[untyped]
        self.update()

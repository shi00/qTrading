...
- CacheCleared 信号经 ``ft.use_state(get_cache_cleared_state)`` 订阅 Observable 自动重渲染
  (UIX-01: 替代原 pubsub 订阅); 导航仍经 ``page.pubsub.send_all_on_topic(TOPIC_NAVIGATE)`` 发送
...
def HomeView(
    ...
    - CacheCleared 经 ``ft.use_state(get_cache_cleared_state)`` 订阅 Observable (UIX-01);
      导航经 pubsub ``TOPIC_NAVIGATE`` 发送

...
项目根目录 `.trae/mcp.json` **尚未创建**（该文件已被 `.gitignore` 排除，为 IDE 本地配置不入版本控制），需用户手动创建以下内容：
...
| AI 未自动加载 MCP server | `.trae/mcp.json` 未创建或路径/格式错误 | 按 §3.1 手动创建 `.trae/mcp.json`；重启 IDE；该文件已被 `.gitignore` 排除需手动维护 |

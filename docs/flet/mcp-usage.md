...
flet-mcp 是 Flet 0.86+ 官方提供的独立 PyPI 包（非 `flet` CLI 子命令），通过 MCP 协议向 AI 暴露**版本特定的 Flet API 权威信息**。服务器对象为 `flet_mcp:mcp`（FastMCP 实例，`mcp.name == "flet-mcp"`），经 `fastmcp run flet_mcp:mcp` 启动。flet-mcp 自带预构建的 `api.json`，**不依赖 flet 运行时安装即可查询 API**（对 CI/纯审查场景友好）。
...
项目根目录 `.trae/mcp.json` **尚未创建**（该文件受 Trae IDE 黑名单保护，AI 不可创建/修改），需用户手动创建以下内容：
...
```json
{
  "mcpServers": {
    "flet": {
      ...
      "args": ["run", "flet_mcp:mcp"]
    }
  }
}
```

**平台路径差异**：Windows 用 `venv/Scripts/fastmcp.exe`；Linux/macOS 用 `venv/bin/fastmcp`。

Trae IDE 启动时自动加载该文件，AI 会话内可直接调用 flet-mcp 工具。服务器对象为 `flet_mcp:mcp`（FastMCP 实例，name="flet-mcp"）。

**依赖前提**：`flet-mcp` 已加入 `pyproject.toml` `[project.optional-dependencies].dev`，需先执行 `pip install -e ".[dev]"` 安装 dev 依赖。fastmcp CLI 入口由 fastmcp 包提供（`venv/Scripts/fastmcp.exe`）。

**worktree 场景**：git worktree 默认不与主工作区共享 venv。在 worktree 内启动 Trae IDE 时，相对路径 `venv/Scripts/fastmcp.exe` 解析为 `<worktree>/venv/Scripts/fastmcp.exe`。若 worktree 内未独立安装 dev 依赖，需在该 worktree 内执行 `pip install -e ".[dev]"`，或改用主工作区 venv 的绝对路径（牺牲跨 worktree 可移植性）。

### 3.2 核心工具清单

| 工具 | 签名 | 用途 | 典型场景 |
|------|------|------|---------|
| `get_api` | `(name, member?, query?, format="text")` | 获取任意 Flet 符号 API 参考 | 不确定 `ft.use_dialog` 签名时；"not found" 即权威否定 |
...
flet-mcp 返回的是 **Flet 官方当前版本行为**，项目契约（`v1-api-constraints.md` / `project-differences.md`）可能在此基础上进一步收窄（如项目统一用 `on_select` 而非 `on_change`）。**项目契约优先**，flet-mcp 用于验证 API 存在性与签名，不用于覆盖项目契约。
...
| 现象 | 原因 | 解决 |
|------|------|------|
| AI 调用 flet-mcp 报「command not found」 | venv 中无 fastmcp（flet-mcp 未装） | `pip install -e ".[dev]"` 安装 dev 依赖 |
| `fastmcp run flet_mcp:mcp` 报 ModuleNotFoundError | venv 中未装 flet-mcp | 同上 |
| AI 未自动加载 MCP server | `.trae/mcp.json` 未创建或路径/格式错误 | 按 §3.1 手动创建 `.trae/mcp.json`；重启 IDE；该文件受 IDE 黑名单保护 AI 不可写 |
| worktree 内 fastmcp.exe 不存在 | worktree 未与主工作区共享 venv | 在该 worktree 内 `pip install -e ".[dev]"`，或改用主工作区 venv 绝对路径 |
...
| Linux 环境下 `fastmcp.exe` 不存在 | 路径平台差异 | Linux 路径为 `venv/bin/fastmcp`，需按 OS 调整 mcp.json |
...
**结论**：§3.2 工具清单签名准确，§3.3 场景 1/2 示例返回值与文档描述一致。

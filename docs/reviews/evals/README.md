...
> **当前定位：人工对照清单，非自动评分。** 每类样例目前仅 1 个 case（共 5 个），样本量在统计上无法支撑比例型阈值。下表阈值保留为**接入自动评分后的目标值**；当前按人工对照逐条核对（AI 报告是否命中期望发现、是否误报、四要素与规则 ID 是否合规）。

| 指标 | 定义 | 目标阈值（接入自动评分后） | 当前用法（人工对照） |
|------|------|--------------------------|---------------------|
| 召回率 | 真实缺陷中被打分为确定缺陷或待验证风险的比例 | ≥ 0.9 | 核对样例的期望发现是否被报告（是/否） |
| 误报率 | 报告为确定缺陷但实际无缺陷的比例 | ≤ 0.1 | 核对无缺陷样例是否被误报为确定缺陷 |
| 证据完整性 | 确定缺陷含位置/触发条件/影响/证据四要素的比例 | = 1.0 | 核对每条确定发现四要素是否齐全 |
| 规则遵从率 | 引用正确规则 ID 且未引用不存在 ID 的发现比例 | = 1.0 | 核对规则 ID 是否取自 ai-review.md 已定义 ID、未有不存在 ID |
...
- 新样例必须声明期望类别、严重度、规则 ID 三要素，以便人工对照核对（后续接入自动评分后用于自动评分）。

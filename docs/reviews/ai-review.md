...
### 4.3 第三轮：反证与验证（ROUND3-01 ~ ROUND3-05）
...
- **[STOP-01]** 可结束检视：范围/意图/基线已明确；高风险路径及触发的专项维度已覆盖；每项发现有足够证据或已降级为待验证风险；重大场景遗漏已识别；已运行和未运行检查被准确记录；且满足下方「STOP-01 停步判据」全部条目（以可核对判据取代原「继续扩大范围的边际收益很低」的定性判断；触及授权边界时改走 STOP-03）。
...
**STOP-01 停步判据**（全部满足才允许停止扩大检视范围；各判据均为可核对信号，不依赖「边际收益」的主观判断）：

| 判据 | 可核对信号 |
|------|-----------|
| 变更规模 | 增量变更文件数 ≤ 5（单点变更，一轮「覆盖→深挖→反证」可完整覆盖）；> 5 个文件时须提供与 STOP-02 等价的范围清单（逐文件已读/明确排除/读取失败）方可停步 |
| 发现收敛 | 最近一轮范围扩展（新增读取文件或风险维度）未产生新的候选发现 |
| 最小验证已跑 | 已按 [CONTRIBUTING.md「变更类型 → 最小验证子集」](../../CONTRIBUTING.md#变更类型--最小验证子集) 选定并运行改动范围对应的最小子集，记录命令与退出状态 |
| 无悬空风险 | 每项候选发现已按 FIND-01 定级，或降级为「待验证风险」逐条登记（含未运行检查项与原因） |
| 未越授权边界 | 未触及 SAFE-02/03 的授权边界；触及时改走 STOP-03 |
...
- **[EVID-01]** 置信度分高/中/低三档，按下表**可核对信号**判定（不凭主观印象），并给出对应产出。
- **[EVID-02]** 低置信度内容通常应列为待验证风险，不应用确定语气形成阻断结论。

**EVID-01 置信度判据**：

| 置信度 | 可核对信号（满足其一） | 报告须给出 |
|-------|---------------------|-----------|
| 高 | 有失败测试/最小复现的运行输出，或完整可达路径（入口→触发点→影响点逐跳可定位） | 复现命令+输出摘要，或逐跳「文件:符号」清单 |
| 中 | 代码、调用关系和契约支持结论但未运行复现 | 支撑结论的「文件:符号」引用清单 |
| 低 | 结论依赖关键未确认假设 | 显式列出的假设清单及未验证原因 |
...
- 三轮检视法（覆盖建模 → 风险驱动深挖 → 反证与验证）已完整执行
- 停止条件（STOP-01~03）已核对；每条发现带 EVID-01/02 证据与 SEV-01/02 严重度
- 输出满足 OUT-01~04，且逐条过完「10. 完成检查表 CHECK-01~15」

_最小验证命令：_ 检视为只读行为，落地产物是检视报告；若修订本文件/契约 → `python scripts/check_docs_consistency.py` + 对应契约测试。

...
| review08-D1 | 新闻抓取两公开方法去重：抽取 _fetch_stock_news_core 共享内核，publish_time 统一为 UTC tz-naive（消除 8h 口径偏差） | review08 数据层与外部依赖 | [news_fetcher.py](../../data/external/news_fetcher.py) / [plans/review08-d1-fix.md](../plans/review08-d1-fix.md) | 使用中 |
| review08-d1 | 即 review08-D1 的小写路径别名（fix/review08-d1-news-dedup 分支名） | 同上（review08-D1） | [plans/review08-d1-fix.md](../plans/review08-d1-fix.md) | 使用中 |

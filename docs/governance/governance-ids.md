...
>
> **维护规则**：新增治理 ID 时必须在下方表格登记一行（由 `check_governance_id_glossary` 门禁守护受检治理文档中出现的 ID——`CHECKED_DOCS` 全集（排除 `CHANGELOG.md` 自动生成发布日志与 `Plans.md` 本地计划）加 `docs/governance/*.yml` 机器可读治理文件；DS-02 起，受检范围扩展至 `scripts/**/*.py` 与 `tests/**/*.py` 的**行注释**中的 ID，未登记 ID 以 `::warning::` 渐进部署，存量清零后翻转 ERROR）；来源轮次不可考的标注「本地检视报告（gitignored）」，不臆造。
>
> **编号格式规范（DS-02）**：每个治理 ID 前缀的编号格式（补前导零与否）必须全局唯一，同一前缀不得混用单位数与补零两种形式（如某前缀下文档用 `-1`、另一文档用 `-01` 引用同一含义时，视为不同 ID、不接受双登记）。新增 ID 遵循下表前缀既定格式；发现旧代码中出现与登记式相反的编号形式时，先核对含义，含义相同则按登记式归一，含义不同则单列。
>
> **UX 与 `UIX-` 前缀区分（DS-02/DS-12）**：`UX-\d+` 是 UI/UX 专项审视轮次的发现编号（如 `UX-07` 全系统 UI/UX 审视轮次）；`UIX-\d+` 是 UI 层检视的独立编号（编码为完整前缀 `UIX-`，请勿与 `UX-` 混写）。两套编号仅差一个前缀字母且当前都在用；登记时严格按实际使用的完整前缀（`UX-`/`UIX-`）填写，`_GOVERNANCE_ID_PATTERN` 已显式区分二者。

...
- **边界**：`AGENTS.md` 不承载 `rule_type` 为 `DEFAULT` / `NEW_CODE` / `MIGRATION_TARGET` / `EXCEPTIONABLE` 的其他红线，避免入口膨胀；这些规则仍由 `CLAUDE.md` / `redlines.yml` 承载。Context 点名的 R1（架构越界）与 R16（UI 阻塞主循环）虽属高成本风险，但不纳入最小安全集，而是接受其在跨工具场景下最多浪费一轮返工的成本，以换取入口不膨胀：R1 由 import-linter 6 条契约在 pre-commit 与 CI 双重拦截、R16 由 pre-commit `redlines-check` hook 部分守护（ViewModel `__init__` 构造已注册单例切面）并在 CI 全量测试暴露，这些兜底在提交阶段生效，能拦截越界代码合入，但救不回 Context 所述的「已浪费的整轮工作」——这正是把它们排除的代价，是刻意接受的权衡而非风险已被兜底消除。最小安全集的定位是「工具链未读取 `CLAUDE.md` 时唯一生效的底线」；收录规则遵循「违反后果不可逆或高成本且无提交阶段兜底」原则，对已有提交阶段自动化拦截的规则不再重复收录（GDR-10）。

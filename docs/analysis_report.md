...
日志 [error.log](file:///d:/workspace/Quantitative%20Trading/astock_screener/logs/error.log) 记录了关键异常：
```
Error loading tables: TableViewerTab Control must be added to the page first
```

**原因链路**：[_lazy_build_ui](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#1140-1203) 执行 `self.update()` 后**立即**调用 `await self.table_tab.did_mount_async()`，Flet 尚未完成子控件挂载，导致 [_toggle_loading(True)](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#359-380) 内的 `self.update()` 抛出异常。由于该调用位于 `try` 块**外部**，`self._is_loading = False` 永不执行，UI 永久锁死在加载状态。
...
| # | 位置 | 修改 | 作用 |
|---|------|------|------|
| 1 | [_toggle_loading](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#359-380) L377 | `self.update()` → `if self.page: self.update()` | 控件未挂载时跳过更新，避免异常 |
| 2 | [_load_schema_and_data](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#381-474) L386 | 将 [_toggle_loading(True)](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#359-380) 移入 `try` 块内 | 确保 `finally` 始终释放 `_is_loading` 锁 |
| 3 | [_load_schema_and_data](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#381-474) finally | [_toggle_loading(False)](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#359-380) 加 `try/except` 保护 | 防止 finally 中再抛异常导致锁无法释放 |
| 4 | [_lazy_build_ui](file:///d:/workspace/Quantitative%20Trading/astock_screener/ui/views/data_view.py#1140-1203) L1182 | 添加 `await asyncio.sleep(0)` | 让出事件循环，确保 Flet 完成子控件挂载 |

## 验证方式

启动应用 → 数据中心 → 数据浏览 → 选择任意表，数据应正常加载显示，不再无限转圈。

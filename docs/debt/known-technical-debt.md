...
| P3-PR513-Script-SysPath-Inject | PR #513 检视：`scripts/verify_embedded_pg_connect.py` 顶部 `sys.path.insert` 注入仓库根后 import data 层（代码味道，已文档化） | P3 | ① `scripts/` 其他脚本出现同类 `sys.path` 注入需统一时；② 或脚本迁移为正式包入口时 |
...
strategies/ 层 except Exception 中 P0/P2 必修项已完成，剩余已合理日志的 except Exception 用 `# NOTE(lazy):` 标记保留。Phase 6.1 杠杆评估决策"不引入 `_handle_strategy_exception(e, context)` 统一入口"（YAGNI：`classify_error` 调用稀少；过度抽象：5 正交维度需 7 字段 context；多处差异化状态修改无法覆盖）。Phase 6.3 完成 ceiling 动态化（按方法分组描述，不再硬编码数值）。R9 合规修复已完成（`str(e)` → `DataSanitizer.sanitize_error(e)`）。
...
R3 场景遗漏检视发现：utils/ 层 except Exception（分布在 config\_handler / exception\_hooks / logger / singleton\_registry / diagnostics 等）。已统一标记 `# NOTE(lazy):`（三要素齐全：简化内容 + ceiling + upgrade），`time_utils.py` 中一处候选标记已剔除（无业务消费方语义，调查时判定不适合标记）。0 处适合走 `classify_error`（均无业务消费方，属基础设施兜底或合理降级）。
...
R1 由 import-linter 6 条契约守护；R4/R12/R13/R14/R15 由 `scripts/check_redlines.py` 实现并接入 pre-commit `redline-check` hook（单元测试守护）。R16（UI 阻塞）部分落地：VM `__init__` 构造已注册单例检测已实现（review07-G20，捕获 B11 类问题）；事件处理器内同步 IO 维度因 AST 误报风险高暂缓，仍需人工评审（重点检查项见 `docs/reviews/ai-review.md`）。各红线 automation\_coverage 与 human\_review\_required 见 `docs/governance/redlines.yml`。
...
存量 ≤5 处或 2026-11-30 前将其转为 ERROR（与弱断言 baseline KPI 节奏一致）。upgrade 触发条件：存量清至 ≤5 处时在 `scripts/check_type_ignore_reason.py` 中翻转 WARNING→ERROR 并更新本条目为已解决。
...
| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-E2E-Windows-Embedded-Timeout-Tight** | E2E Tests (Windows) embedded 模式 step `timeout-minutes` 余量不足（embedded 模式 > 45 分钟 / 原 45 分钟 = 100% 占用） | ① E2E Tests (Windows) embedded 模式实际运行时间 ≥ 55 分钟（92% 占用 60 分钟 timeout）；② 或连续 3 次 PR 触发 E2E Tests (Windows) 超时；③ 或 E2E 测试总数增长 ≥ 20%（如新增 9 个测试） |

**产生背景与现状**

PR #291 方案 B 将 E2E Tests (Windows) 从外置 PostgreSQL 改为 embedded 模式（spec.md §3 不变量 1：embedded 为默认）。外置 PostgreSQL 模式（main 分支）总时长 31 分钟（run 30151665758，2026-07-25），原 step `timeout-minutes: 45` 余量 14 分钟（\~31%）。embedded 模式需要构建 sidecar binary + PG 16 initdb + 44 个 E2E 测试，总时长 > 45 分钟（run 30183636080 在 45 分钟 timeout 时被取消，未完成）。embedded 模式比外置 PostgreSQL 模式慢的原因：① sidecar 构建（Cargo build）约 3-5 分钟；② PG 16 initdb + sidecar 启动约 5-10 分钟；③ embedded 模式下 DB 操作可能比外置 PostgreSQL 慢。本次 PR #291 修复将 step `timeout-minutes` 从 45 提升至 60（余量 \~15 分钟 / \~25%），属短期止血；长期需优化 E2E Tests 性能或拆分 job。根因在 embedded 模式的资源密集特性（每个 Flet 子进程启动 sidecar + PG 16 initdb），与 Windows runner 性能波动叠加易突破 timeout。相关文件：`.github/workflows/ci_cd.yml` 的 `e2e-tests-windows` job、`tests/e2e/conftest.py`。
...
① sidecar `run.rs` supervise 循环新增 postgres 日志监听 task（持续读 postgres stderr/log file），匹配 `invalid page` / `checksum failure` 关键字后经 `protocol::print_json_line(&EventJson::warning("data_page_corruption", ...))` 上报 stdout；② Python `_stdout_reader_task` 解析 `event.warning` 事件并经 ViewModel state 通知 UI；③ UI 显示数据页损坏警告 + 引导备份恢复；④ `failure_injection.rs` 新增 `test_inject_25_data_page_checksum_error` 集成测试（篡改测试表数据页 → 查询 → 验证 sidecar stdout warning 事件）。验收标准：① sidecar 监听 postgres 日志并上报 `data_page_corruption` warning 事件；② Python + UI 正确显示警告；③ 集成测试覆盖 #25 场景；④ 不破坏现有 stdout 协议向后兼容。upgrade 触发条件：① sidecar 引入运行期 postgres 日志监控需求时；② 或数据页损坏问题在生产环境报告时。

#### P3-M9-EmbeddedPg-TimeoutExpired-Propagation：#M9-013 错误处理缺口：embedded\_pg\_maintenance\_service.py subprocess TimeoutExpired 未分类处理
...
| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-CON04-Subprocess-Cancel** | review02 CON-04：subprocess 不可取消长任务停机交互——terminate 仅终止 sidecar 包装 / dump 中断残留不自动清理 / restore 拒绝取消 / run\_async 无可中断变体 | ① 用户报告停机后 `.partial` 残留或孤儿 pg\_dump 占用磁盘/连接；② restore 取消成为产品需求；③ ThreadPoolManager 重构时 |

**产生背景与现状**

review 02 检视发现 `_run_sidecar` 用 `subprocess.run(timeout=3600)` 不可取消，停机时被进程退出孤立强杀残留 `.partial` 备份文件。CON-04 最小修复（2026-09-03）：改用 `Popen` 暴露句柄 + 新增 `request_cancel()`，`utils/shutdown.py` Step 6 在关闭线程池前主动终止在途 dump（`cancel_allowed=True` 仅 dump 类注册）。**已知限制（对抗性检视 2026-09-03 确认）**：`terminate()` 仅终止 sidecar 包装进程（sidecar 维护命令未装信号处理器，maint.rs/pgbin.rs 无 SIGTERM 传播），在线路径（PG 运行中直连 pg\_dump）下孤儿 `pg_dump` 可能继续写 `.partial`，直到其自然结束 / Step 8 停止 PG / 进程退出回收；dump 中断 `.partial` 残留仅由 sidecar doctor `dump_partials` 检测、不自动清理。**推迟项（YAGNI）**：① dump 中断 `.partial` 残留自动清理；② restore 取消（数据一致性优先，拒绝取消）；③ `ThreadPoolManager.run_async` 可中断变体。相关文件：`services/embedded_pg_maintenance_service.py`、`utils/shutdown.py`、`sidecars/qtrading-pg-sidecar/src/maint.rs`/`pgbin.rs`。

**期望的最终解法**

期望最终解法：① sidecar 为维护命令安装 SIGTERM/SIGINT 处理并传播到 `pg_dump` 子进程（`kill_on_drop` 仅覆盖优雅路径，需显式信号处理器）；② dump 中断时删除 `.partial` 或 doctor 提供清理动作；③ 评估 restore 可安全取消点；④ `run_async` 支持可中断变体。验收标准：① terminate 后 `pg_dump` 进程树被终止、无 `.partial` 继续增长；② sidecar dump 中断无 `.partial` 残留或 doctor 可清理；③ restore 取消有明确一致性边界；④ run\_async 中断语义有单测覆盖。upgrade 触发条件：① 用户报告停机后 `.partial` 残留或孤儿 pg\_dump 占用磁盘/连接；② restore 取消成为产品需求；③ ThreadPoolManager 重构时。

#### P3-PR513-Script-SysPath-Inject：PR #513 检视：`scripts/verify_embedded_pg_connect.py` 顶部 `sys.path.insert` 注入仓库根后 import data 层（代码味道，已文档化）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-PR513-Script-SysPath-Inject** | PR #513 检视：`scripts/verify_embedded_pg_connect.py` 顶部 `sys.path.insert` 注入仓库根后 import data 层（代码味道，已文档化） | ① `scripts/` 其他脚本出现同类 `sys.path` 注入需统一时；② 或脚本迁移为正式包入口时 |

**产生背景与现状**

PR #513 检视（第一轮）遗留观察级建议。`scripts/verify_embedded_pg_connect.py:26-32` 因脚本经 `python scripts/verify_embedded_pg_connect.py` 直接运行时 `sys.path[0]` 指向 `scripts/`，data 包不在可导入路径上，故用 `sys.path.insert(0, str(_REPO_ROOT))` 将仓库根注入 `sys.path` 后 `import data.persistence.embedded_postgres.version`。注入后 `data/__init__.py`、`data/persistence/__init__.py` 均用惰性 `__getattr__`，不触发重依赖，且非红线 `data/cache` 路径，可接受。`sys.path` 注入为代码味道。相关文件：`scripts/verify_embedded_pg_connect.py:26-32`。

**期望的最终解法**

声明脚本为可安装入口（如 `pyproject.toml` 的 `[project.scripts]`）或以包内相对导入方式运行，消除 `sys.path` 注入。验收标准：① 移除 `sys.path.insert` 后 `python -m scripts.verify_embedded_pg_connect` 可运行；② 现有使用方式（`python scripts/verify_embedded_pg_connect.py`）不回退。upgrade 触发条件：① `scripts/` 其他脚本出现同类 `sys.path` 注入需统一时；② 或脚本迁移为正式包入口时。

#### P3-PR513-VersionCache-No-Prod-Invalidation：PR #513 检视：`_version_cache` 生产期不主动失效（合理设计，供未来评估）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-PR513-VersionCache-No-Prod-Invalidation** | PR #513 检视：`_version_cache` 生产期不主动失效（合理设计，供未来评估） | ① 应用引入运行期 sidecar 升级/替换流程时；② 或 EmbeddedPostgresService 支持热切换时 |

**产生背景与现状**

PR #513 检视（第一轮）遗留观察级建议。`data/persistence/embedded_postgres/version.py` 的模块级 `_version_cache` 由 `_version_lock` 保护临界区，缓存仅缓存成功结果，同一进程生命周期内假定 sidecar 二进制不随版本变更。`clear_pg_version_cache()` 已提供手动失效入口，但生产路径不主动调用：应用运行期间若 sidecar 被重新安装为不同 PG 主版本，缓存不会自动失效，需外部显式调用 `clear_pg_version_cache()`。当前应用运行期不替换 sidecar，属合理设计。相关文件：`data/persistence/embedded_postgres/version.py`。

**期望的最终解法**

若应用运行期引入 sidecar 热替换/升级流程，在替换后自动调用 `clear_pg_version_cache()`。验收标准：① sidecar 替换后下次 `resolve_pg_major_version` 重新解析；② 现有 `test_version.py` 全过；③ 新增 sidecar 替换后缓存失效单测。upgrade 触发条件：① 应用引入运行期 sidecar 升级/替换流程时；② 或 EmbeddedPostgresService 支持热切换时。

### 其他 P3 债目

#### P3-App-ExcInfo-Traceback-Path-Leak：`exc_info=True` 的 traceback 文件路径未经 sanitize\_error 脱敏（合理设计决策，供未来评估）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-App-ExcInfo-Traceback-Path-Leak** | `exc_info=True` 的 traceback 文件路径未经 sanitize\_error 脱敏（合理设计决策，供未来评估） | ① 安全审计要求脱敏文件路径时；② 或引入自定义 logging Formatter 时 |

**产生背景与现状**

M3 模块化检视发现：`app/error_logging.py`（`log_exception_with_severity` 函数）与 `app/bootstrap.py` 8 处 `_log(..., exc_info=True)` 调用时，logging 模块从 `sys.exc_info()` 提取 traceback 附加到日志记录，traceback 含文件绝对路径（如 `D:\workspace\qTrading\app\bootstrap.py`）。`DataSanitizer.sanitize_error` 默认 `show_traceback=False`（已确认 `utils/sanitizers.py` 的 `DataSanitizer.sanitize_error` 默认值），只处理 exception 消息字符串，不处理 `exc_info` 附加的 traceback。检视结论：文件路径泄露不是 R9 红线违规（R9 定义为"明文 Token/API Key/密码/个人信息"，文件路径不属于这些类别）；`exc_info=True` 是调试必需的（traceback 对问题定位至关重要），当前实现是"调试可见性"与"信息泄露风险"之间的合理权衡。本次 M3 修复了 `detail=str(e)` → `DataSanitizer.sanitize_error(e)`（FIND-01），但未触及 `exc_info` 的 traceback 路径。相关文件：`app/error_logging.py`（`log_exception_with_severity` 函数）、`app/bootstrap.py`（`_log` 调用处）、`app/startup_controller.py`（`_init_services` 与 `skip` 方法）、`utils/sanitizers.py`（`DataSanitizer.sanitize_error` 的 `show_traceback` 默认值）。

**期望的最终解法**

如需统一脱敏 traceback 路径：① 自定义 logging Formatter（在日志输出层统一处理 traceback 路径脱敏）；② 或在 `sanitize_error` 中支持 `exc_info` 参数自动提取并脱敏 traceback。验收标准：① traceback 中的文件路径经脱敏后为 `<PATH>`；② 现有 `test_error_logging.py` 全过；③ 新增 traceback 路径脱敏单测。upgrade 触发条件：① 安全审计要求脱敏文件路径时；② 或引入自定义 logging Formatter 时。

#### P3-App-SyncFunctions-In-Async-Bootstrap：`initialize_services` 中同步函数（preload\_aliases/validate\_failover/validate\_strategy\_tier）阻塞事件循环（合理设计决策）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-App-SyncFunctions-In-Async-Bootstrap** | `initialize_services` 中同步函数（preload\_aliases/validate\_failover/validate\_strategy\_tier）阻塞事件循环（合理设计决策） | ① 这些函数引入网络/磁盘 IO；② 或启动期 UI 响应延迟被用户感知时 |
...
不主动修复。若未来 `preload_aliases` / `validate_failover_credentials` / `validate_strategy_tier_coverage` 引入网络 IO 或耗时 CPU 操作（如 >100ms），改用 `ThreadPoolManager().run_async(TaskType.IO, ...)` 提交。验收标准：① 改造后启动期 UI 无阻塞；② 现有 `test_bootstrap.py` 全过。upgrade 触发条件：① 这些函数引入网络/磁盘 IO；② 或启动期 UI 响应延迟被用户感知时。

#### P3-M12-FailoverConfigPanel-BtnSave-No-RunTask：#M12-023 一致性：failover\_config\_panel.py btn\_save 未通过 \_run\_task\_no\_args 提交
...
M12 ui 表现层模块检视发现：`ui/components/config_panels/failover_config_panel.py` `btn_save` 未通过 `_run_task_no_args` 提交，与同模块其他按钮模式不一致。M12 未修复原因：修复需评估 btn\_save 是否需要异步提交，属行为变更。相关文件：`ui/components/config_panels/failover_config_panel.py`。
...
| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-PR373-Viewport-Collapse-Audit** | 增量变更触发边缘状态视口塌陷同类风险排查（PR #373 举一反三） | ① 上述视图新增控件触发 E2E 文本超时失败时；② 或布局重构时 |
...
排查 3 个文件的 `Row(wrap=True)` + `expand=True` 组合是否存在边缘状态视口风险：① 用 Playwright DOM dump 测量各视图 main 基线表体视口高度；② 若存在边缘状态（接近 0px），评估新增控件触发塌陷的概率；③ 必要时调整布局（如 `expand=2` 或独立右对齐行）。验收标准：① 3 个视图 main 基线视口高度已测量并记录；② 边缘状态视图已修复或标记 NOTE(lazy)；③ 现有 E2E 测试全过。upgrade 触发条件：① 上述视图新增控件触发 E2E 文本超时失败时；② 或布局重构时。

#### P3-PR3-TextButton-Interactive-Unverified：PR-3 对抗性检视 M3：ft.TextButton + AnchorKind.INTERACTIVE 的 CanvasKit 行为未验证

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-PR3-TextButton-Interactive-Unverified** | PR-3 对抗性检视 M3：ft.TextButton + AnchorKind.INTERACTIVE 的 CanvasKit 行为未验证 | ① 新增使用 `click_skip` 的测试时；② 或 `ft.TextButton` 升级时 |
...
| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-PR3-MutPool-Locale-Restore-No-Fallback** | PR-3 对抗性检视 M6：test\_settings\_language\_switch finally 块还原失败时 mut pool 内连坐污染 | ① CI 出现 mutates\_config 测试连坐失败时；② 或 mut pool 测试数量增长 ≥50% 时 |

**产生背景与现状**

PR-3 删除 `_ensure_locale_zh` 后，`flet_app_mut` pool 的 locale 还原仅依赖 `test_settings_language_switch` finally 块。若 UI 操作失败（`sp.select_language(lang_zh)` 超时），`flet_app_mut` 内存 locale 停留 en\_US，影响后续 `mutates_config` 测试。**设计权衡**：Plans.md Task 3.4 明确要求删除 `_ensure_locale_zh`，read-only 池不受影响。测试进程 I18n 已由 `I18n.set_locale("zh")` 在 UI 操作前还原。相关文件：`tests/e2e/test_settings_flow.py:67-88`、`tests/e2e/conftest.py`。

**期望的最终解法**

在 `pristine_config` teardown 时为 mut pool 增加 locale 还原兜底（类似原 `_ensure_locale_zh` 但限定 mut pool）。验收标准：① finally 块失败后 mut pool 后续测试仍能正常运行；② read-only pool 不受影响。upgrade 触发条件：① CI 出现 mutates\_config 测试连坐失败时；② 或 mut pool 测试数量增长 ≥50% 时。

#### P3-B12-Screener-Formatting-Logic-in-VM：#B12-2 review02 B12 第 2 步：格式化逻辑下沉 VM（`_format_cell_value`/`_COLUMN_WIDTHS`/`_HIDDEN_COLS` 从 View 迁到 VM）
...
review02 B12 已实现第 1 步（data\_version memo 缓存，消除无谓重算，解决 90%）；第 2 步需将 `_format_cell_value`/`_COLUMN_WIDTHS`/`_HIDDEN_COLS` 从 View 迁到 VM，state 携带渲染就绪数据。跨 VM 边界且与 report04 D9（AI 流式更新重建机制重构）"避免 data\_version 未变时全量重建"目标耦合，故本批次只实现第 1 步，第 2 步登记为依赖 D9 的后续任务（修复方案 §B12 第 2 步）。相关文件：`ui/views/screener_view.py`。
...
| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-M4-DbMigrator-OrphanHeal-SingleChain-Assumption** | `_heal_orphaned_revision` 假设严格线性单链迁移（已文档化设计限制） | 引入 Alembic 多分支（Multiple Heads）时 |
...
#### P3-Windows-Unit-Test-Timeout-Tight：Windows 单测 step `timeout-minutes` 余量不足（main 成功 run 13 分 41 秒 / 原 15 分钟 = 91% 占用）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-Windows-Unit-Test-Timeout-Tight** | Windows 单测 step `timeout-minutes` 余量不足（main 成功 run 13 分 41 秒 / 原 15 分钟 = 91% 占用） | ① Windows 单测实际运行时间 ≥ 18 分钟（90% 占用 20 分钟 timeout）；② 或连续 3 次 PR 触发 Windows 单测超时；③ 或单测总数增长 ≥ 20%（如新增 2000 个测试） |

**产生背景与现状**

PR #291 触发 Windows 单测 CI 超时调查发现：main 分支最近一次成功 Windows 单测（run #1004，2026-07-25）实际运行 821s（13 分 41 秒），原 step `timeout-minutes: 15` 余量仅 79s（\~9%）。main 分支 run #999（2026-07-25 04:48）已实际触发 timeout（运行 909s / 15 分 9 秒，conclusion=failure）。PR #291 触发的 Windows 单测运行 911s（15 分 11 秒）同样触发 timeout。Windows runner 性能波动 + 测试增量（如 PR #291 新增 14 个 mock 单测 + 默认值变更让部分测试走更长路径）易突破 timeout。本次 PR #291 修复将 step `timeout-minutes` 从 15 提升至 20（余量 \~6.5 分钟 / \~32%），属短期止血；长期需优化慢测试或拆分 job。根因不在 PR #291 的代码变更，而在 CI 基础设施层面的余量设计。相关文件：`.github/workflows/ci_cd.yml` 的 `Run Unit Tests` step（Windows job）、`tests/unit/`。

**期望的最终解法**

长期优化方案（任选其一或组合）：① 用 `pytest --durations=20` 识别 main 分支最慢的 20 个测试并优化（如 mock 重型 IO、减少 fixture 重复初始化）；② 将 `tests/unit/` 拆分为 2-3 个并行 job（如按 `tests/unit/data/`、`tests/unit/ui/`、`tests/unit/strategies/` 等目录拆分），缩短 wall clock；③ 将 `pyproject.toml` 中 `--dist=loadgroup` 切换为 `--dist=loadscope` 减少跨 worker fixture 重复初始化开销。验收标准：① main 分支 Windows 单测实际运行时间 ≤ 12 分钟（余量 ≥ 8 分钟 / \~40%）；② 拆分 job 后各 job wall clock ≤ 8 分钟；③ 现有测试全过且无新增 flaky。upgrade 触发条件：① Windows 单测实际运行时间 ≥ 18 分钟（90% 占用 20 分钟 timeout）；② 或连续 3 次 PR 触发 Windows 单测超时；③ 或单测总数增长 ≥ 20%（如新增 2000 个测试）。

### UI 表现层（MVVM/单例/可访问性）

#### P3-M11-StartupViews-HomeViewModel-Static-Call：#M11-002 MVVM 边界：startup\_views.py View 直接调用 HomeViewModel 静态方法
...
M12 ui 表现层模块检视发现：`ui/components/stock_detail_dialog.py` View 直接持有 `page` 和 `data_processor` 引用（过渡期设计）。严格 MVVM 边界下，View 不应直接持有 page/data\_processor，应通过 ViewModel 消费。M12 未修复原因：修复需引入 StockDetailViewModel，跨多文件协调，属 MVVM 完全改造。相关文件：`ui/components/stock_detail_dialog.py`。
...
M12 ui 表现层模块检视发现：`ui/views/screener_view.py` 32 个 import，`ui/views/data_view.py` 24 个 import（2026-09-04 复核），导入复杂度过高。高导入复杂度通常表明文件职责过重。M12 未修复原因：修复需拆分文件职责，属大型重构。相关文件：`ui/views/screener_view.py`、`ui/views/data_view.py`。
...
M12 ui 表现层模块检视发现：VM 中重复调用 `TushareClient()` / `TaskManager()` 等单例（每次调用走 `@register_singleton` 的 `__new__` 路径）。虽然单例模式保证只创建一个实例，但重复调用有轻微性能开销。M12 未修复原因：修复需在 VM `__init__` 中缓存单例引用，但需评估生命周期（VM dispose 时是否需要释放）。相关文件：`ui/viewmodels/` 下多个文件。
...
#### P3-DataExplorer-SyncEngine-Unification：DataExplorer 同步引擎游离于统一连接管理（举一反三登记，本期不实施）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-DataExplorer-SyncEngine-Unification** | DataExplorer 同步引擎游离于统一连接管理（举一反三登记，本期不实施） | ① DataExplorer 再次出现连接错口/引擎缓存跨模式复用问题；② 或统一连接管理重构时 |
...
将 `DataExplorerQueryClient._shared_engine` 归一化到统一连接管理：复用受统一生命周期管理的引擎（或经统一提供者获取），移除自建 sync 引擎缓存与独立 ShutdownCoordinator 注册，并处理 sync/async 双态 + 线程池建连语义。验收标准：① `_shared_engine` 从 DataExplorerQueryClient 移除/复用统一引擎；② DataExplorer 查询路径仍恒定命中当前 DB(embedded/external) URL；③ 现有 `test_data_explorer*` / `test_data_db_manager*` / e2e 数据页全过；④ 新增归一化后同步/异步引擎共用回归测试。upgrade 触发条件：① DataExplorer 再次出现连接错口/引擎缓存跨模式复用问题；② 或统一连接管理重构时。

#### P3-UX06-Startup-SLA-E2E-Gap：冷启动到导航可交互的端到端 SLA 未实测（UX-06 以 proxy 构成基线替代，如实记录缺位）

| 级别 | 一句话 | upgrade 触发条件 |
|------|--------|------------------|
| **P3-UX06-Startup-SLA-E2E-Gap** | 冷启动到导航可交互的端到端 SLA 未实测（UX-06 以 proxy 构成基线替代，如实记录缺位） | ① 真实端到端测量 > 8s（SLA 超阈）时触发 visited\_tabs 顶层推广（`test_consumes_all_seven_subviews_in_stack` 改写 + #438 范式推广）；② 或启动流程重构时 |

**产生背景与现状**

UX-06（P1-04 冷启动验证）实测生产模式全页构造 proxy 成本（`scripts/probe/startup_perf.py` 可复现）：依赖库 import \~5.4s（本机 dev 基准，pandas 主导）+ 单例 \~0.46s + 7 View 渲染 \~0.16-1.03s ≈ 6.5-7s；litellm 已由 `ai_service._ensure_litellm_loaded` 双层惰性移除 18s+ 阻塞（实测 litellm 不在 import/构造链）。visited\_tabs 懒构造推广判定为收益/风险比不成立（可省 \~1-1.4s 仅 15-20%，主导成本 import 不可省；且转嫁首次切页冻结 + 契约测试改写）。**缺位**：端到端真实冷启动→NavigationRail 可交互测量（含 DB/embedded-PG 初始化、低性能机器）需 GUI/浏览器环境，无头无法执行，SLA（定义 8s 阈值）状态按 proxy 构成判定达标，未以真实启动实测佐证。相关文件：`scripts/probe/startup_perf.py`、`ui/app_layout.py`（\_build\_pages\_stack）、`app/bootstrap.py`（已删 E2E 无效预热）、`UX-06-方案设计.md`（worktree 内）。

**期望的最终解法**

在 GUI/浏览器环境下执行真实启动测量：启动应用 → `time NavigationRail 可交互`（Flet 首帧 patch 下发时间戳）。验收标准：① 真实冷启动 ≤ 8s（或按实测更新 SLA 阈值）；② 低性能机器同样测量并记录。upgrade 触发条件：① 真实端到端测量 > 8s（SLA 超阈）时触发 visited\_tabs 顶层推广（`test_consumes_all_seven_subviews_in_stack` 改写 + #438 范式推广）；② 或启动流程重构时。
...
`strategies/backtest/data_provider.py::_BacktestQualityProxy` 为让回测历史任意时点数据通过质量门控，硬编码 `QualityTier.GOLD` 绕过 `@require_quality`/`_check_tier`（CLAUDE.md §3.2 强制要求）。结果：回测使用的历史数据（含缺失日/异常值/未复权段）不反映数据质量问题，策略 `required_quality_tier` 声明形同虚设，与 UN-04「可信地验证」冲突。完成第一步（代码处 `# NOTE(lazy)` 三要素显式标记 + 本条目登记）后，权衡对维护者可见。
...
`SwIndustryMember` 是全局快照（对应 Tushare `index_member_all`），主键不含日期。回测 2021 年用的是**今天**的行业归属，申万分类整体调整（如 2021 年三级体系修订）时，跨调整期的回测会把"当时不属于该行业"的股票算进去，影响行业中性/行业分散度上限/行业轮动类策略。**决策**：不修（申万成分历史需额外接口与配额成本，属产品决策，review03 建议按需再评估）。**缓解**：回测报告 UI 摘要显式标注"行业分类为当前快照，跨分类调整期的回测可能存在前视"（`backtest_result_panel.py::_build_chart_summary` + i18n `backtest_chart_summary_industry_note`）。**关联验证**：DAT-08①（`LATERAL` 加 `ORDER BY index_code`）已消除同一快照内的行选择不确定（PR #710），本条仅针对跨期前视。相关文件：`data/persistence/daos/screener_dao.py`（行业 `LATERAL` 子查询）、`data/sync/sw_industry.py`、`ui/components/backtest/backtest_result_panel.py`。

#### P3-DAT08-IndustryPollution：存量污染：`stock_basic.industry` 在 DAT-08③ 前被写时覆写为申万二级行业，拆列后 `industry_tushare` 输出仍含旧覆写值（review03 DAT-08③）
